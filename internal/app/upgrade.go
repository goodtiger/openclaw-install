...
func runUpgrade(args []string, out, errOut io.Writer) error {
	...
	assetURL := fmt.Sprintf("https://github.com/goodtiger/openclaw-install/releases/download/v%s/%s", latestVersion, assetName)
	...
	if runtime.GOOS == "windows" {
		downloadURL = assetURL + ".zip"
	} else {
		downloadURL = assetURL + ".tar.gz"
	}

	fmt.Fprintln(out, "下载中...")
	downloadedFile, err := downloadFile(ctx, downloadURL, tmpDir, out)
	if err != nil {
...
func getLatestReleaseInfo(ctx context.Context, out io.Writer) (string, string, error) {
	req, err := http.NewRequestWithContext(ctx, "GET", "https://api.github.com/repos/goodtiger/openclaw-install/releases/latest", nil)
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", "", fmt.Errorf("GitHub API 返回错误状态: %s", string(body))
	}
...
func verifyChecksum(downloadedFile string, version string, dir string, out io.Writer) error {
	checksumURL := fmt.Sprintf("https://github.com/goodtiger/openclaw-install/releases/download/v%s/SHA256SUMS", version)
